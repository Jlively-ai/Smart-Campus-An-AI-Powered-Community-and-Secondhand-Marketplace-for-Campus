<!--
  组件名：GoodsCategoryIndex
  功能描述：商品分类管理页面
  主要职责：展示商品分类列表，支持新增、编辑、删除分类等操作
-->
<template>
  <div>
    ...
    <el-table :data="list" v-loading="loading" border @selection-change="handleSelectionChange">
      ...
      <el-table-column prop="goodsCategoryId" label="ID" width="60" align="center"></el-table-column>
      <el-table-column prop="goodsCategoryName" label="分类名称" min-width="200"></el-table-column>
      <el-table-column label="操作" width="120" align="center">
        <template slot-scope="scope"><el-button size="mini" type="danger" @click="handleDelete(scope.row.goodsCategoryId)">删除</el-button></template>
      </el-table-column>
    </el-table>
  </div>
</template>
<script>
import { getGoodsCategoryList, addGoodsCategory, deleteGoodsCategory } from '@/api/manage'
export default {
  /** 组件数据定义 */
  ...
  methods: {
    ...
    async handleAdd() {
      ...
      await addGoodsCategory(this.newCategoryName)
      this.$message.success('添加成功'); this.newCategoryName = ''; this.loadData()
    },
    /** 删除记录 */
    async handleDelete(goodsCategoryId) {
      try { await this.$confirm('确定删除该分类？', '提示', { type: 'warning' }) } catch { return }
      await deleteGoodsCategory(goodsCategoryId); this.$message.success('删除成功'); this.loadData()
    },
    /** 表格选中行变化回调 */
    ...
    async batchDelete() {
      ...
      for (var i = 0; i < this.selectedRows.length; i++) { await deleteGoodsCategory(this.selectedRows[i].goodsCategoryId) }
